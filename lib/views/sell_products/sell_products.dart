...
class _SellProductsState extends State<SellProducts> {
  ...
  Widget build(BuildContext context) {
    return BlocListener<SellProductsBloc, SellState>(
      listener: (context, state) {
        if (state is SellAddPrdLoading) {
          loadingWidget(context);
        } else if (state is SellAddProdSuccess) {
          navigatorKey.currentState?.pop();

          alertWidget(context, "product successfully added",
              title: "Success", textColor: Colors.green);
            ...
            onTap: () {
              if (titleController.text.isEmpty ||
                ...
                context.read<SellProductsBloc>().add(SellProductsEvent(
                    ...
                    smallSize: int.parse(smallController.text),
                    mdSize: int.parse(mediumController.text),
                    lgSize: int.parse(lgController.text),
                    xlSize: int.parse(xlController.text),
                    img1: image1!,
                    img2: image2!,
                    img3: image3!,
                    img4: image4!,
